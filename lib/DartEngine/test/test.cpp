...
TEST_CASE("GameSettings")
{
    SUBCASE("output")
    {
        ...
        CHECK_EQ(json, R"({"players":[{"id":1,"points":301},{"id":2,"points":301}],"round":{"attempts":0,"scored_points":0},"style":0,"winners":[]})");
    }
    SUBCASE("output2")
    {
        GameSettings settings;
        auto json = settings.output().dump();
        CHECK_EQ(json, R"({"players":[],"round":{"attempts":0,"scored_points":0},"style":0,"winners":[]})");
    }
    SUBCASE("input")
    {
        nlohmann::json json = nlohmann::json::parse(R"({"players":[{"id":1,"points":301},{"id":2,"points":301}],"round":{"attempts":2,"scored_points":31},"style":0,"winners":[{"id":4,"points":0}]})");
        ...
        CHECK_EQ(settings.round.attempts, 2);
        CHECK_EQ(settings.round.scored_points, 31);
    ...
    SUBCASE("input2")
    {
        ...
        settings.input(json);
        CHECK_EQ(settings.style, GameStyle::Classic);
        CHECK_EQ(settings.round.attempts, 0);
        CHECK_EQ(settings.round.scored_points, 0);
        int player_size = players.size();
        for (int i = 0; i < player_size; ++i)
        {
            Player& p1 = settings.players.front();
...
TEST_CASE("Game")
{
    SUBCASE("constructor/output settings")
    {
        ...
        CHECK_EQ(settings.round.attempts, 3);
        CHECK_EQ(settings.round.scored_points, 0);
    }
    ...
    {
        GameSettings settings;
        settings.round.attempts = 2;
        settings.round.scored_points = 31;
        ...
        CHECK_EQ(settings2.round.attempts, 2);
        CHECK_EQ(settings2.round.scored_points, 0);
